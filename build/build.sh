...
# 更新 package.json 版本号（移除前缀v）
version_number=${currentTag#v}
sed -i "s/\"version\": \"[^\"]*\"/\"version\": \"$version_number\"/" package.json
echo "Updated package.json version to: $version_number"

# 如果参数是 update-version-only，则只更新版本号，不执行git操作
if [ "$1" == "update-version-only" ]; then
  echo "Version updated only, skipping git operations"
  exit 0
fi
