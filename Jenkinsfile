    ...
    stage('更新版本号') {
      steps {
        script {
          ...
          // 只更新版本号，不创建git tag
          sh "./build/build.sh update-version-only"
        }
      }
    }
    ...
    stage('推送打包文件到仓库') {
      steps {
         script {
          ...
          def GIT_URL = "https://${PROJECT_TOKEN_GK}:${PROJECT_TOKEN}@e.coding.net/serverless-100013832940/picture/picture_admin.git"
          if (BRANCH_ID == 'master') {
            sh "chmod +x ./build/build.sh"
      ...
      steps {
        script {
          ...
          if (BRANCH_ID == 'test-master') {
            ...
            DEPLOY_DIR = '/home/ubuntu/workspace/ai/test/admin'
            sh 'echo "测试环境"'
          }
          //预发布分支
          if (BRANCH_ID == 'release') {
            ...
            CRED_HOST = "${TEST_REMOTE_HOST}"
            CRED_PORT = "${TEST_REMOTE_SSH_PORT}"
            DEPLOY_DIR = '/home/ubuntu/workspace/ai/release/admin'
            sh 'echo "预发布环境"'
          }
           //正式分支
          if (BRANCH_ID == 'master') {
            ...
            DEPLOY_DIR = '/home/ubuntu/workspace/ai/admin'
          }
          ...
          withCredentials([usernamePassword(
            ...
            sshCommand(
              remote: remoteConfig,
              command: "rm -rf ${DEPLOY_DIR}/assets",
              sudo: true
            )
            sshPut(
              remote: remoteConfig,
